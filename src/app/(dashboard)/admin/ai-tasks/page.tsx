...
export default function AITasksPage() {
  ...
  async function refreshProviderModels(provider: string) {
    try {
      ...
      if (res.ok) {
        ...
        showSuccess(`${provider} models refreshed`);
      }
    } catch (err) {
  ...
  async function saveMultiModelConfig() {
    ...
    try {
      ...
      const data = await res.json();
      setMultiModelConfig(data);
      showSuccess("Multi-model configuration saved");
    } catch {
      setError("Failed to save multi-model configuration");
    } finally {
    ...
    <div className="space-y-6">
      ...
      <div className="flex items-center justify-between">
        <div className="flex items-center gap-3">
          ...
          <div>
            <h1 className="text-2xl font-bold text-zinc-900 dark:text-white">AI Configuration</h1>
            <p className="text-sm text-zinc-500 dark:text-zinc-400">
              Configure AI providers and models for each task
            </p>
          </div>
        </div>
        <Button
            ...
            >
              <div className="flex items-center justify-between">
                <div>
                  ...
                  <p className="text-sm text-zinc-500 dark:text-zinc-400">
                    {hasKey ? `${modelCount} models` : "No API key"}
                  </p>
                </div>
                <button
              ...
              {isExpanded && isEditing && editingConfig && (
                <div className="space-y-4 border-t border-zinc-200 p-4 dark:border-zinc-700">
                  ...
                  <div>
                    ...
                    {!hasApiKey && (
                      <p className="mt-1 text-sm text-amber-600 dark:text-amber-400">
                        Add API key in Settings to load models from API
                      </p>
                    )}
                  </div>
                  ...
                  <div className="flex items-center justify-between rounded-lg bg-zinc-50 p-3 dark:bg-zinc-700/50">
                    <span className="font-medium text-zinc-900 dark:text-white">Enabled</span>
                    <button
                      onClick={() =>
                        setEditingConfig({
                          ...editingConfig,
        ...
        <div className="rounded-lg border border-violet-200 bg-violet-50 dark:border-violet-800 dark:bg-violet-900/20">
          ...
          >
            <div className="flex items-center gap-3">
              ...
              <div>
                <div className="flex items-center gap-2">
                  <h3 className="font-semibold text-zinc-900 dark:text-white">Multi-Model Review</h3>
                ...
                <p className="text-sm text-zinc-500 dark:text-zinc-400">
                  {multiModelConfig.models.length} models configured
                </p>
              </div>
            </div>
            <ChevronDown
          ...
          {multiModelExpanded && (
            <div className="space-y-4 border-t border-violet-200 p-4 dark:border-violet-700">
              ...
              <div className="flex items-center justify-between rounded-lg bg-white p-3 dark:bg-zinc-800">
                <span className="font-medium text-zinc-900 dark:text-white">Enable Multi-Model Review</span>
                <button
                  onClick={() =>
                    setMultiModelConfig({
                      ...multiModelConfig,
              ...
              <div>
                <div className="mb-2 flex items-center justify-between">
                  <Label className="text-sm font-medium">Review Models</Label>
                ...
                <div className="space-y-2">
                  ...
                  {multiModelConfig.models.length === 0 && (
                    <div className="rounded-lg border-2 border-dashed border-zinc-300 p-4 text-center text-sm text-zinc-500 dark:border-zinc-600">
                      No models configured. Add at least 2 for multi-model review.
                    </div>
                  )}
                </div>
              </div>
              ...
              {/* Synthesis model */}
              <div>
                <Label className="mb-2 block text-sm font-medium">Synthesis Model</Label>
                <div className="flex gap-2">
                  <select
              ...
              {/* Min models */}
              <div>
                <Label className="mb-2 block text-sm font-medium">Minimum Models Required</Label>
                <Input
                  type="number"
                  min={2}
                  max={5}
